package com.ysw.aipassagecreator.agent.tools;

import com.ysw.aipassagecreator.model.dto.image.ImageRequest;
import com.ysw.aipassagecreator.model.enums.ImageMethodEnum;
import com.ysw.aipassagecreator.service.CosService;
import com.ysw.aipassagecreator.service.ImageServiceStrategy;
...
public class ImageGenerationTool {
     ...
     */
    //@Tool把普通 Java 方法，暴露成「大模型可以主动调用的工具 / 函数」@ToolParam标记方法入参 = 工具参数说明

    /**结合你当前 @Tool + @ToolParam 的架构，AI 自主识别、生成、组装并传递参数，整套流程和落地写法如下，分原理、配置、实战、调试四部分讲。
     * 框架会把 @Tool 方法 + 所有 @ToolParam 的描述、参数名、类型，自动整理成工具元数据（函数定义）发给大模型。
     * 大模型根据自身任务（比如 “给文章配封面图”），结合参数说明，自主判断需要哪些参数、该填什么值。
     * AI 按照约定格式生成 JSON 格式参数，框架自动解析、反射调用你的 Java 方法。
     * 你的代码只负责执行业务，不用手动传参、不用解析 AI 参数。
     */
    //专供 AI Agent 自动调用
